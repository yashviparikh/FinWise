...
function Watchlist({ userid = 1 }) {
  ...
  const authConfig = useMemo(
    () => (uid ? { headers: { "X-User-Id": String(uid) } } : {}),
    [uid]
  );
  ...
  const fetchWallet = useCallback(() => {
    if (!uid) return;
    axios
      .get(`http://127.0.0.1:5000/get_wallet/${uid}`, authConfig)
      .then((res) => {
  ...
  const fetchWatchlist = useCallback(() => {
    if (!uid) return;
    axios
      .get(`http://127.0.0.1:5000/get_watchlist/${uid}`, authConfig)
      .then((res) => {
  ...
  const trackStock = (symbolRaw) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        return axios.post(
          "http://127.0.0.1:5000/add_to_watchlist",
          {
            userid: uid,
            stock_id,
          },
  ...
  const removeTracked = (stockId) => {
    ...
    axios
      .post(
        `http://127.0.0.1:5000/remove_from_watchlist/${uid}/${stockId}`,
        null,
        authConfig
      )
      .then(() =>
  ...
  const executePurchase = () => {
    ...
    axios
      .post(
        "http://127.0.0.1:5000/buy_from_watchlist",
        {
          userid: uid,
